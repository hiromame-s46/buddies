# Buddies — 櫻坂46ファン向けコミュニティマッチングプラットフォーム

Buddies（バディーズ）は、櫻坂46を推している人同士がつながるためのファンコミュニティです。プロフィール交換、ユーザー検索、投稿タイムライン、コミュニティ運営、お問い合わせ管理をひとつのサービスで利用できます。

## 📱 主な機能

### 🏠 ホーム
アプリの入口です。ログイン状態に応じて利用できる操作が変わります。
- **未ログイン時**: 最初にログイン画面が表示され、ログイン・アカウント作成・ゲストで続けるを選べます。ゲストはプロフィール閲覧など一部機能のみ利用できます
- **ログイン済み**: タイムライン、探す、マイページ、プロフィール交換などを利用できます

### ✉️ メール認証
メール認証はBuddies単体で動作します。認証リンクは24時間有効・一度だけ使用でき、認証済みアドレスからパスワードを再設定できます。

- 新規登録ウィザードの最後、または `/account` から任意で設定
- メールアドレス変更時は新しいアドレスを再認証
- DB接続は `api.php` が `../../../api/config.php` を読み込み、スキーマ作成も同API内で行います
- SMTPなどの設定は Buddies ディレクトリ内の `.env.example` を `.env` にコピーして設定します。メール送信・暗号化・認証トークン・本文生成はBuddies内で完結し、`../mail-system` は実行時に読み込みません
- `BUDDIES_MAIL_TOKEN_SECRET` は必ず本番用の32文字以上のランダムな秘密値に変更してください

### 📰 タイムライン
ユーザーやコミュニティの投稿を閲覧・作成できます。

- 全体タイムラインとフォロー中タイムライン
- テキスト、画像、URL、ハッシュタグを含む投稿
- SakuLaboブログ、SakuMapスポット、さくみみの引用
- いいね、コメント、フォロー
- 投稿詳細の共有URLとプロフィール別投稿一覧
- 投稿後15分以内の編集、削除、通報
- 投稿画像の送信前軽量化

### 👤 マイページ
自分のプロフィールを確認・編集できます。

#### プロフィール表示
- **基本情報**: 表示名、性別、活動地域、年齢
- **推し情報**: 
  - 推し始めた時期
  - 推しメン（最大3人、1番目が第一推し）
  - 好きな曲（最大3曲）
- **自己紹介**: 好きなこと、活動エリアなどを自由に記入
- **興味タグ**: ライブ参戦、グッズ推奨など（最大5個）
- **SNSリンク**: X、Threads、Instagram、TikTok、YouTube、カスタムリンク（最大4個）
  - SNS設定済みの場合、プロフィール上部に「SNS登録済み」バッジが表示されます
- **フォローのスタンス**:
  - 無言フォローOK
  - フォロー前に一言欲しい
- **紹介タグ**: X上でワンタップで投稿できるプロフィール紹介テンプレート
- **お気に入りブログ**: SakuLaboでお気に入りにしたブログの公開表示
- **投稿一覧**: プロフィールからそのユーザーの投稿を確認

#### プロフィール編集
「編集」ボタンからプロフィールを編集できます。

**基本情報編集**
- 表示名（20文字まで）
- 性別（男性/女性/その他/未選択）
- 活動地域（都道府県選択）
- 自己紹介（500文字まで）

**推し情報編集**
- 推し始めた時期（年選択）
- 推しメン追加: 推しメンの名前を入力するとサジェストが表示されます。最大3人まで追加可能。1番目が第一推しになります。
- 好きな曲追加: 曲名を入力するとサジェストが表示されます。入力欄の上に候補リストが表示されます。ひらがな・カタカナは区別なく検索できます。最大3曲まで追加可能。
- 興味タグ追加: タグを入力するとサジェストが表示されます。同様にひらがな・カタカナは区別なく検索できます。最大5個まで追加可能。

**SNS編集**
- SNSの種類を選択（X、Threads、Instagram、TikTok、YouTube、カスタムリンク）
- URLを入力
- SNSリンクが1つ以上あると、「フォローのスタンス」設定が表示されます
- 最大4個まで追加可能

**紹介タグ編集**
- テンプレート選択:
  - **スタンダード**: 基本情報（地域・性別・世代）、推しメン、フォロー時のお願いを含むフル版
  - **シンプル**: 推しメン情報のみのシンプル版
  - **にぎやか**: 絵文字を多用した賑やかな版
  - **カスタム**: 自分で自由に編集可能
- プロフィール画面の「X」ボタンをタップすると、このテンプレートテキストがクリップボードにコピーされ、Xで投稿できます

### 🔍 探す
同じ推し、共通の興味を持つ人を見つけられます。

#### 検索
- **テキスト検索**: 名前、推しメン名、興味タグ、好きな曲で検索できます
- **クリアボタン**: 入力欄の右側に×ボタンが表示され、タップするとすぐに検索欄をクリアできます

#### フィルター
「フィルター」ボタンでより細かい条件で絞り込めます。複数のタブから条件を指定できます。

**メンバータブ**
- マッチング条件: 「いずれか」（1人でも一致）、「全員一致」（全員推している）を選択可能
- 推しメンを複数選択して絞り込み

**興味タブ**
- 登録されている人気の興味タグから選択
- タグが追加されると、そのタグを選択している人が表示されます
- 自分で入力して追加するボタンはありません（登録されているタグのみから選択）

**楽曲タブ**
- 登録されている人気の楽曲から選択
- 楽曲が追加されると、その曲を好きな人が表示されます
- 自分で入力して追加するボタンはありません（登録されている楽曲のみから選択）

**SNSタブ**
- 「SNS設定済みのみ表示」をオンにすると、SNSリンクを登録している人のみが表示されます

#### 検索結果
マッチした人のプロフィールカードが表示されます。
- スクロールで自動的に次のページが読み込まれます（無限スクロール）
- プロフィールカードをタップするとプロフィール詳細モーダルが開きます

### 👥 プロフィール詳細
検索結果やマイページからプロフィールカードをタップすると詳細モーダルが開きます。

#### 表示内容
- 表示名とプロフィール画像
- SNS登録済みバッジ（設定済みの場合）
- 基本情報（性別、地域、年齢）
- マッチ理由の表示（共通の推しメンや興味タグがあれば「〇〇推し同士」などと表示）
- My推し情報（推し始めた時期、推しメン、好きな曲）
- 自己紹介文
- 興味タグ
- SNSリンク（クリック/タップで外部サイトへ）
- プロフィール画像（タップで拡大表示）

#### 操作
- 推しメン名や好きな曲をタップすると、その条件で検索できます
- 興味タグをタップすると、そのタグで検索できます
- SNSアイコンをタップすると、SNSプロフィールへジャンプします
- 共有ボタン: Xでシェアできます（プロフィール情報テキストがツイート欄に挿入されます）

### 🤝 プロフィール交換
Buddiesアプリ内のQRコードで、対面した相手とプロフィールを相互交換できます。

- 自分の交換用QRコードを表示
- アプリ内カメラで相手のQRコードを読み取り
- 交換済みプロフィールをマイページに保存
- 交換済みかどうかをプロフィール上に表示
- 通常の共有URLは閲覧専用とし、交換はアプリ内読み取りに限定

### 🏢 コミュニティアカウント
イベントや企画を運営する公式・協力アカウント向けの機能です。

- コミュニティ／開発者アカウントの専用プロフィール
- イベント、フォーム、掲示板への導線
- コミュニティからのお知らせ投稿
- 一般ユーザーの検索・おすすめへの統合表示

### 🎨 プロフィール画像設定
マイページの画像をタップして、プロフィール画像を変更できます。

- アイコンピッカーが開き、複数のデザインから選択可能
- 背景色とアイコンスタイルを組み合わせてカスタマイズ
- 初期設定では自動生成されたアイコンが表示されます

### ⚙️ アカウント設定
マイページの「設定」ボタンから、アカウント関連の設定ができます。

- **ユーザー名変更**: 英数字・アンダースコア使用可（3〜32文字）
- **パスワード変更**: 8文字以上の新しいパスワードを設定
- **メール認証**: 任意でメールアドレスを登録・変更できます。登録・変更ごとに24時間有効の認証リンクで確認します
- **パスワードリセット**: メール認証済みユーザーは、ログイン画面からパスワードを再設定できます
- **ログアウト**: このデバイスからサインアウト

### ✉️ お問い合わせ
チャット形式で問い合わせを送信し、ログインユーザーは履歴と返信をサイト内で確認できます。

- 返信の要否、サイト内返信／DM、匿名表示を問い合わせごとに設定
- カテゴリと対象サイトを指定
- 初回・追加メッセージへの画像添付
- 条件内での本文編集、問い合わせの取りやめ、対応済み項目の非表示
- 管理者との追加メッセージ履歴と未読表示
- 認証済みメールアドレスへの返信通知
- 管理者は返信ごとにメール通知の有無を選択

## 💾 データ管理

### プロフィール情報の保存
- すべてのプロフィール情報は自動的に保存されます
- 編集画面で「保存」ボタンをタップすると変更が確定します
- 「キャンセル」で編集を破棄できます

### 削除機能
- 各項目（推しメン、好きな曲、興味タグ、SNS）の×ボタンで個別に削除可能

## 🌐 ブラウザ対応

- モバイルブラウザ（iOS Safari、Android Chrome等）推奨
- デスクトップブラウザも対応（Chrome、Safari、Firefox等）
- 日本語IME対応（変換中の誤送信を防止）

## 🔒 安全性と運用

- セッション認証と管理者権限の分離
- 投稿・お問い合わせの送信間隔制限
- 投稿通報と管理画面での対応
- 共有URLとQR交換フローの分離
- 取り消せない操作を実行する前の確認表示

## 📝 ヒント

初めてプロフィールを作成するときは、以下の情報を入力することをお勧めします：

1. **推しメンを設定**: 同じメンバーを推している人が見つけやすくなります
2. **好きな曲を追加**: 音楽の好みで繋がれます
3. **興味タグを設定**: ライブ参戦、グッズ推奨などの活動スタイルを共有
4. **自己紹介文を記入**: あなたの人柄が伝わりやすくなります
5. **SNSリンクを設定**: フォローしやすくなります

---

## サポート

不具合報告、機能リクエスト、情報提供は、[お問い合わせフォーム](../form/)からご連絡ください。
